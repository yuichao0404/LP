...
以下の商品情報を基に、楽天市場で10年以上の実績を持つトップセラーのライティング担当者として、検索エンジン、SNS、比較サイト、口コミサイトなど、あらゆる外部流入経路から訪れるユーザーに刺さる、SEOに強く、人間味のある自然な文章でLPを作成してください。
...
**役割**：ヒーロー画像のイメージを想起させるシーン（商品は映さない）
...
The scene should evoke the same feeling as the hero image but from a different angle or perspective. 
No products visible, pure atmospheric representation that supports the main brand message. 
Japanese minimalism, editorial magazine quality, sophisticated and calming. 
--ar 16:9 --style raw --v 6
```

**パラメータ調整ガイド（ワーカー向け）**：
- ヒーロー画像と同じテーマで異なるアングルを表現
- ブランドの一貫性を保ちながらバリエーションを付ける
- 同じ色彩や雰囲気で統一感を演出
